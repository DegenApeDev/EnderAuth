...
class LoginAndRegister implements Listener{
    ...
    public function onJoin(PlayerJoinEvent $event){
    	...
    	elseif($this->status !== "enabled" and $this->isOp($event->getPlayer())){
    		$event->getPlayer()->sendMessage("[EnderAuth] There is an error, and EnderAuth has been disabled!");
    	}
    	if($this->plugin->status === "enabled" and $this->plugin->provider === "yml"){
        	...
        	if(!$this->plugin->registered->exists(strtolower($event->getPlayer()->getName()))){
                ...
                $event->getPlayer()->sendMessage("[EnderAuth] You are not registered.");
                $event->getPlayer()->sendMessage("[EnderAuth] Type your wanted password in chat.");
        		return;
            }
            else{
                if($this->plugin->getConfig("ip-auth") === true && $myuser->get("myip") !== $event->getPlayer()->getAddress()){
                    ...
                    $event->getPlayer()->sendMessage("[EnderAuth] Your IP does not match.");
                    $event->getPlayer()->sendMessage("[EnderAuth] Please type your password in chat.");
                    return;
                }
                if($this->plugin->getConfig("ip-auth") === true && $myuser->get("myip") === $event->getPlayer()->getAddress()){
                    $event->getPlayer()->sendMessage("[EnderAuth] You are now logged-in.");
                    return;
                }
                if($this->plugin->getConfig("ip-auth") !== true){
                    $event->getPlayer()->sendMessage("[EnderAuth] Please type your password in chat to log-in.");
                    return;
                }
        	    else{
